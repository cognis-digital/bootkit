...
**bootkit** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `bootkit` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
bootkit ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
bootkit ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
bootkit ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `bootkit` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="bootkit") for rec in records]   # records = your JSON output
